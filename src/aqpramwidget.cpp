 ...
 */
AQParamWidget::AQParamWidget(UASInterface* uas_ext, QWidget *parent) :
    UASParamManager(uas_ext, parent),
...
void AQParamWidget::changeEvent(QEvent* event)
{
...
void AQParamWidget::retranslateUi() {
...
void AQParamWidget::saveSettings() {
    QSettings settings;
...
void AQParamWidget::loadParameterInfoCSV(const QString& autopilot, const QString& airframe)
{
    Q_UNUSED(airframe);
 ...
 */
UASInterface* AQParamWidget::getUAS()
{
    return mav;
}
 ...
 */
void AQParamWidget::addComponent(int uas, int component, QString componentName)
{
    Q_UNUSED(uas);
 ...
 */
void AQParamWidget::addParameter(int uas, int component, int paramCount, int paramId, QString parameterName, QVariant value)
{
    ...
    if (justWritten && !writeMismatch && missWriteCount == 0)
    {
        ...
        pal.setColor(backgroundRole(), colorGreen);
    ...
    } else if (justWritten && writeMismatch)
    {
        // Mismatch, tell user
        QPalette pal = statusLabel->palette();
        pal.setColor(backgroundRole(), colorRed);
        statusLabel->setPalette(pal);
    ...
    else
    {
        ...
        {
            QPalette pal = statusLabel->palette();
            pal.setColor(backgroundRole(), colorOrange);
            statusLabel->setPalette(pal);
        }
        else
        {
 ...
 * @param parameterName human friendly name of the parameter
 */
void AQParamWidget::addParameter(int uas, int component, QString parameterName, QVariant value)
{
    ...
    if (OverrideCheckValue && changedValues.contains(component) && changedValues.value(component)->contains(parameterName)) {
        parameterItem->setBackground(0, QBrush(QColor(colorOrange)));
        parameterItem->setBackground(1, QBrush(QColor(colorOrange)));
...
void AQParamWidget::parameterItemChanged(QTreeWidgetItem* current, int column)
{
    if (!current || !column)
        return;
    ...
    if (map) {
        ...
        if (!parameters.value(key)->contains(str) || fabs(static_cast<float>(parameters.value(key)->value(str).toDouble()) - value.toDouble()) > 2.0f * FLT_EPSILON) {
            current->setBackground(0, QBrush(QColor(colorOrange)));
            current->setBackground(1, QBrush(QColor(colorOrange)));
...
void AQParamWidget::saveParamFile()
{
...
void AQParamWidget::saveParameters(int fileFormat)
{
    if (!mav) return;
...
void AQParamWidget::loadParameters()
{
    if (!mav) return;
 ...
 */
void AQParamWidget::setRetransmissionGuardEnabled(bool enabled)
{
    if (enabled) {
...
void AQParamWidget::retransmissionGuardTick()
{
 ...
 */
void AQParamWidget::requestParameterUpdate(int component, const QString& parameter)
{
 ...
 */
void AQParamWidget::setParameter(int component, QString parameterName, QVariant value)
{
 ...
 */
void AQParamWidget::setParameters()
{
    // Iterate through all components, through all parameters and emit them
 ...
 */
void AQParamWidget::writeParameters()
{
...
void AQParamWidget::readParameters()
{
    if (!mav) return;
 ...
 */
void AQParamWidget::clear()
{
    tree->clear();
    components->clear();
}

void AQParamWidget::trackExpandedItems(QTreeWidgetItem *item)
{
...
QVariant AQParamWidget::getParaAQ(QString parameterName)
{
...
void AQParamWidget::setParaAQ(QString parameterName, QVariant value)
{
    setParameter(190, parameterName, value);
}

void AQParamWidget::loadParaAQ()
{
    requestParameterList();
}


void AQParamWidget::loadParaFromSD()
{
    if (uas != NULL)
    {
...
void AQParamWidget::saveParaToSD()
{
    if (uas != NULL)
    {
...
void AQParamWidget::restartUasWithPrompt()
{
    QMessageBox msgBox;
...
void AQParamWidget::calibrationAccStart()
{
    if (!mav)
        return;
...
void AQParamWidget::calibrationMagStart()
{
    if (!mav)
        return;
...
void AQParamWidget::calibrationSave()
{
    if (!mav)
        return;
...
void AQParamWidget::calibrationLoad()
{
    if (!mav)
        return;
...
void AQParamWidget::loadOnboardDefaults()
{
    if (!mav) return;
...
void AQParamWidget::setRestartBtnEnabled(const bool enable)
{
    restartButton->setEnabled(enable);
}

void AQParamWidget::setCalibBtnsEnabled(const bool enable)
{
...
void AQParamWidget::setFilePath(QString fileName)
{
    fileNameFromMaster = fileName;
}

void AQParamWidget::resetLinkLossExpected() {
    //    if (uas != NULL) {
    //        for ( int i=0; i < uas->getLinks()->count(); i++)
...
void AQParamWidget::commandAckReceived(int uasid, int compid, uint16_t command, uint8_t result)
{
    Q_UNUSED(compid)
