...
void DebugConsole::setLinkState(LinkInterface *link)
{
    ...
    m_ui->receiveText->appendHtml(QString("<span style=\" font-weight:bold\">" + time + "</span>"  + " -- " + "Link %1 %2.\n").arg(link->getName(), state));
...
void DebugConsole::receiveTextMessage(int id, int component, int severity, QString text)
{
    ...
    if (isVisible())
    {
        ...
        QTime curtime = QTime::currentTime();
        QString time = curtime.toString("hh:mm:ss");
        m_ui->receiveText->appendHtml(QString("<span style=\" font-weight:bold\">" + time + "</span>"  + " -- " + "(%1) %2 \n").arg(name, text));
    ...
    if (link == currLink && !holdOn)
    {
        ...
        if (lineBuffer.length() > 0 && (QGC::groundTimeMilliseconds() - lastLineBuffer) > 200) {
            if (isVisible())
            {
                ...
                m_ui->receiveText->appendHtml(QString("<span style=\" font-weight:bold\">" + time + "</span>"  + " -- " + lineBuffer));
...
void DebugConsole::hold(bool hold)
{
    if (holdOn != hold) {
        ...
        if (this->holdOn && !hold) {
            ...
            m_ui->receiveText->appendHtml(QString("<span style=\" font-weight:bold\">" + time + "</span>"  + " -- " + QString(holdBuffer)));
