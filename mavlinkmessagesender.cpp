#include "ui_mavlinkmessagesender.h"
#include "mavlinkmessagesender.h"
#include "mavlinkprotocol.h"
#include <QWidget>

MAVLinkMessageSender::MAVLinkMessageSender(MAVLinkProtocol* mavlink, QWidget *parent) :
    QWidget(parent),
    protocol(mavlink),
    ui(new Ui::MAVLinkMessageSender)
{
    ui->setupUi(this);
    ...
    connect(ui->sendButton, SIGNAL(pressed()), this, SLOT(sendMessage())); //@Leo : send config msg
}

void MAVLinkMessageSender::refresh()
{
    // Send messages
...
bool MAVLinkMessageSender::sendMessage()
{
    return sendMessage(ui->messageIdSpinBox->value());
}

bool MAVLinkMessageSender::sendMessage(unsigned int msgid)
{
...
MAVLinkMessageSender::~MAVLinkMessageSender()
{
    delete ui;
}

void MAVLinkMessageSender::createTreeView()
{
...
void MAVLinkMessageSender::createField(int msgid, int fieldid, QTreeWidgetItem* item)
{
