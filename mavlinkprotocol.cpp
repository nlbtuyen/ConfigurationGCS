...
//    enableLogging(settings.value("LOGGING_ENABLED", m_loggingEnabled).toBool());
    ...
    // Set auth key
    m_authKey = settings.value("AUTH_KEY", m_authKey).toString();
    enableAuth(settings.value("AUTH_ENABLED", m_authEnabled).toBool());
...
void MAVLinkProtocol::storeSettings()
{
    ...
    settings.setValue("AUTH_KEY", m_authKey);
    settings.setValue("AUTH_ENABLED", m_authEnabled);
    if (m_logfile)
    {
...
//                }
            ...
            if (uas == NULL && message.msgid == MAVLINK_MSG_ID_HEARTBEAT)
            {
                ...
                uas = MAVLinkUASFactory::createUAS(this, link, message.sysid, &heartbeat);
            }
